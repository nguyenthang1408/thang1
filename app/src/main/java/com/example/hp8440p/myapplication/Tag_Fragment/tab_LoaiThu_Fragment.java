...
public class tab_LoaiThu_Fragment extends Fragment {
    ...
    public static NoiDungLoaiThu_Adapter adapter;
    Database database;
...
//
                // lấy thông tin
                   NoiDung nd = new NoiDung(noidung,soTien,theLoai,nGay,ghiChu); // truyền vô tham số
                arrayList.add(nd);
                    if(database.addData(nd) > 0 ){
    ...
    public void getDataList(ListView lv){
        database = new Database(getActivity());
        adapter = new NoiDungLoaiThu_Adapter(database.getDataLT(), this,R.layout.item_loaithu_fragment, lv);
